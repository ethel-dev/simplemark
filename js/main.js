...
function papercode() {
  ...
  if (document.getElementById("sourcecode").getAttribute("disabled") === "disabled" && document.getElementById("gist").getAttribute("disabled") === "disabled") {
    // Check for FileReader support.
    if (window.File && window.FileReader && window.FileList && window.Blob) {
      // Classify input.
  ...
  } else if (document.getElementById("fileupload").getAttribute("disabled") === "disabled" && document.getElementById("sourcecode").getAttribute("disabled") === "disabled") {
    var gistid = $("#gist").val();
    $.ajax({
      url: 'https://api.github.com/gists/' + gistid,
      type: 'GET',
